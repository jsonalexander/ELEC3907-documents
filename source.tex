...
\begin{document}
	...
	\section{Project Overview}
		...
		The goal of this project is to provide a compact arcade-style game console designed to encourage hands-on interaction commonly found in traditional arcade systems.
	...
	\section{Design}
		...
		\subsection{System-Level Description}
			...
			\subsubsection{The Hardware}
				...
				There are two LCD screens for game selection, acting as a menu for players to interact with the console.
				These LCD screens will also act as instructional screens to guide players through the games.
				There are two removable and adjustable controllers, where the controller can be plugged into either side of the console to accommodate right- and left-handedness or for single and multiplayer options.
				The speaker provides sound effects for audio feedback and soundscapes during gameplay.
				There is also a freestanding divider that will be used during games when the players should not see each other's portions of the display---as in the case in battleship, for example.
				...
				Finally, the power supply powers the display and the microcontrollers, the latter of which subsequently powers the other components.
			...
			\subsubsection{The Software}
				...
				To overcome scheduling issues, especially with continuous-delivery applications like audio playback, the second microcontroller is dedicated to receiving playback signals from the first microcontroller and controlling the speaker ansynchronously.
				This second microcontroller can be much less powerful than the main one, essentially serving as an additional thread for the console, allowing it to avoid audio cut-outs and maintain responsiveness.

\newpage

				Additionally, since the hardware system is designed to be general-purpose, the group has decided to leverage object-oriented programming techniques in C++ to model the hardware system and allow for composability.
		...
		\subsection{Preliminary Specifications and Requirements}
			...
			\subsubsection{The Outputs}
				...
				The display will be a 16x16 grid of LEDs which will serve as the main display for the game console.
				An addressable RGB LED strip will be cut into 16 lines of 16 LEDs, the lines being wired together neatly on a piece of plywood.
				The LEDs will be controlled by an Arduino Mega and require an external power supply.
				...
				The enclosure will also need a thin 1 foot by 1 foot piece of plywood to serve as a base, a styrofoam sheet to make a grid separating the LEDs, and frosted acrylic glass to diffuse light.
				Most of these items can be found in the lab, with others being provided by group members.
				The only items to be purchased and included in our costs will be the LED strip at around \$25, the plywood between \$15 and \$30, the styrofoam sheet around \$2, and the acrylic glass being around \$5.
		...
		\subsection{Design Approaches}
			...
			\subsubsection{The Divider}
				...
				\noindent If it is a challenge to make the divider stand upright on its own, a slot can be made in the display housing to help hold the divider in place.
...
\newpage 
	...
	\section{Risk and Safety Assessment}
		...
		\subsection{Power delivery}
			...
			The recommended power supply from the vendor is 60 Watts (5 Volts at 12 Amps), however initial testing has shown that the actual power needed for this project is likely to be much less than that since all the LEDs in the matrix will not be at full load when portions of the display are off and only individual colours (i.e. red, green, blue) are used instead of combinations thereof.
	...
	\section{Sustainability and Lifecycle}
		...
		Additionally, since the codebase will be built on generic classes modelling the hardware, games can be added further down the line to improve replayability by flashing new programs onto the microcontrollers.
	...
	\section{Team Roles and Responsibilities}
		...
		While these responsibilities have been assigned, it is expected that the group will need to work together to ensure the project goes according to plan and its many parts interoperate properly.
...
\newpage
	...
	\section{Project Timing}
		...
		The following work breakdown structure lists all foreseen tasks with major design phases in chronological order.
